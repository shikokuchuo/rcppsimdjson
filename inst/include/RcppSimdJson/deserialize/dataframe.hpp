...
inline auto build_col(simdjson::ondemand::array                       array,
    ...
    auto out   = Rcpp::Vector<RTYPE>(array.count_elements(), na_val<R_Type>());
    ...
    if (type_doc.is_homogeneous()) {
        if (type_doc.has_null()) {
            for (auto object : array) {
                ...
                if(object.get_object().find_field_unordered(key).get(value) == simdjson::SUCCESS) {
                    out[i_row] = get_scalar<scalar_T, R_Type, HAS_NULLS>(value);
                }
                i_row++;
            }
            ...
            for (auto object : array) {
                simdjson::ondemand::value value;
                if(object.get_object().find_field_unordered(key).get(value) == simdjson::SUCCESS) {
                    out[i_row] = get_scalar<scalar_T, R_Type, NO_NULLS>(value);
                }
                i_row++;
            }
        }
        ...
        for (auto object : array) {
            simdjson::ondemand::value value;
            if(object.get_object().find_field_unordered(key).get(value) == simdjson::SUCCESS) {
                out[i_row] = get_scalar_dispatch<RTYPE>(value);
            }
            i_row++;
        }
    }
        ...
        if (type_doc.is_homogeneous()) {
            if (type_doc.has_null()) {
                for (auto object : array) {
                    simdjson::ondemand::value value;
                    if(object.get_object().find_field_unordered(key).get(value) == simdjson::SUCCESS) {
                        stl_vec[i_row] = get_scalar<int64_t, rcpp_T::i64, HAS_NULLS>(value);
                    }
                    i_row++;
                }
...
build_data_frame(simdjson::ondemand::array                                                        array,
    ...
    const auto n_rows    = R_xlen_t(array.count_elements());
